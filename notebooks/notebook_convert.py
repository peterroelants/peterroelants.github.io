...
JEKYLL_POST_URL_LINK_RE = re.compile(r"\[([^\]]+)\]\((\{% post_url [^%]+%\}(?:#[^)]+)?)\)")
...
def enrich_images(soup: BeautifulSoup, add_dimensions: bool, base_dir: Path) -> None:
    ...
    """
    # Optional import of PIL only when dimensions are requested
    Image = None
    if add_dimensions:
        try:
    ...
    for img in soup.find_all("img"):
        ...
        if add_dimensions and Image is not None and not img.has_attr("width"):
            src = img.get("src", "")
            if isinstance(src, str) and src.startswith("/images/"):
                ...
                try:
                    ...
                    # Ignore if image cannot be opened or not found
                    pass
...
def add_notebook_source_note(soup: BeautifulSoup, nbpath: Path) -> None:
    ...
    cell = soup.new_tag("div", attrs={"class": "cell border-box-sizing text_cell rendered"})
    prompt = soup.new_tag("div", attrs={"class": "prompt input_prompt"})
    inner_cell = soup.new_tag("div", attrs={"class": "inner_cell"})
    rendered = soup.new_tag("div", attrs={"class": "text_cell_render border-box-sizing rendered_html"})
...
def get_arguments() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--nbpath", type=str, help="File path of notebook file to convert to blogpost")
    parser.add_argument("--date", type=str, help="Date of original publication of post.")
    parser.add_argument(
        "--layout",
        type=str,
    ...
    parser.add_argument("--subdir", type=str, default="", help="Sub directory of base dir to put post.")
    parser.add_argument("--tags", nargs="+", type=str, default=[], help="Tags related to the post.")
    parser.add_argument(
        "--redirect_from",
        nargs="*",
        default=[],
    ...
    parser.add_argument(
        "--add_img_dimensions",
        action="store_true",
        help=("If set, attempt to add width/height to local images based on file dimensions."),
    )
    parser.add_argument(
        "--add_notebook_source_note",
        action="store_true",
        help=("If set, append a standard footer linking to the generated post and source notebook."),
    )
    return parser
...
def run(args: argparse.Namespace):
    ...
    enrich_images(soup, add_dimensions=args.add_img_dimensions, base_dir=BASE_DIR)
    ...
    html_str = soup.decode(formatter="html")
    # Add Jekyll header
