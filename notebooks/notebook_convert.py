...
def preserve_jekyll_post_url_links(notebook):
    ...
    for cell in notebook.cells:
        ...
        cell["source"] = JEKYLL_POST_URL_LINK_RE.sub(
            r'<a href="\2">\1</a>', source
        )
    return notebook
...
def get_arguments() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Major update in DATE|TITLE|DESCRIPTION format. Can be repeated."
        ),
    )
    parser.add_argument(
        "--add_img_dimensions",
        action="store_true",
        help=(
            "If set, attempt to add width/height to local images based on file dimensions."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If set, append a standard footer linking to the generated post and source notebook."
        ),
    )
    return parser
