...
class RationalQuadratic(psd_kernel.PositiveSemidefiniteKernel):
  ...
  def __init__(
    ...
    with tf.name_scope(
      ...
      tf.assert_same_float_dtype([self._amplitude, self._length_scale, self._scale_mixture_rate])
    super(RationalQuadratic, self).__init__(feature_ndims, dtype=dtype, name=name)
