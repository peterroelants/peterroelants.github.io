...
- Search discovery: XML sitemap plus optional manual IndexNow notifications
...
2. Script uses `nbconvert` to HTML and `BeautifulSoup` post-processing:
   ...
   - Removes stderr-only output areas while preserving normal output in mixed
     output areas
   - Optionally extracts notebook PNG, JPEG, and SVG outputs plus supported
     embedded image, video, and audio data URLs into generated files under
     `images/notebook_outputs/` and references them from the generated post
     with normal media URLs. Filenames include a short content hash, and the
     exact per-notebook asset directory is replaced on each successful
     conversion to avoid stale files; extracted images receive intrinsic
     dimensions.
   - Optionally replaces inline BokehJS and supported Bokeh CDN loader URLs with
     version-pinned local BokehJS bundles under `js/bokeh/` for generated pages;
     notebook execution is unchanged. Published Bokeh output is standalone
     browser JavaScript and must not depend on a Bokeh/Python server
   - Optionally extracts standalone Bokeh outputs from cells explicitly tagged
     `lazy-bokeh` into hashed JavaScript assets and adds the shared
     `js/notebook_bokeh_lazy.js` loader. The loader starts those outputs as they
     approach the viewport; untagged Bokeh outputs remain inline.
...
- Externalized notebook images also receive intrinsic `width` and `height`
  attributes to reserve layout space while they load.
...
- `just browser-test` runs the focused Playwright regression check for the
  lazy Bokeh path against the built static site. It is separate
  from `just check` because it requires a locally installed Chromium browser.
...
- GitHub Actions rebuilds CSS before handing the site to GitHub Pages' managed
  Jekyll builder.
- After the Jekyll build, GitHub Actions runs the focused browser regression
  check for the lazy Bokeh implementation before uploading the Pages artifact.
- IndexNow notifications are manual after major deployed changes; the key file
  and submission policy are documented in `build_info/indexnow.md`.
