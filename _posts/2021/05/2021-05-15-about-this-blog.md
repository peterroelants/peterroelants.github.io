...
- Blog
- Notebook
image: /images/social/2021-05-15-about-this-blog-preview.png
updates:
- date: 2026-09-01
  title: Documented interactive charts and notebook media
  description: Explained how standalone Bokeh JavaScript and extracted notebook media keep the site interactive and lightweight.
---
...
Some posts use [Bokeh](https://bokeh.org/) for interactive charts and small
browser-based demonstrations. GitHub Pages only serves static files, so a
published chart cannot rely on a Python process or a Bokeh server. We therefore
publish these outputs as standalone Bokeh documents. The page includes the
chart data and JavaScript callbacks, while [BokehJS](https://docs.bokeh.org/en/latest/docs/user_guide/advanced/bokehjs.html)
renders the chart or app and handles its interactions in the visitor's browser.

### Extracting notebook media

Notebooks typically store their outputs directly in the notebook document.
Exporting that document can copy the outputs into the HTML, making pages with
many plots, GIFs, or other media unnecessarily large. Our conversion script
extracts notebook-generated images and supported embedded media, such as GIFs,
videos, and audio, then references those files from the HTML instead. Browsers
can cache the extracted files and load them lazily where appropriate, while the
converter preserves their alt text. The notebook remains the source of truth;
extraction is simply one step in converting an executed notebook into a post.
