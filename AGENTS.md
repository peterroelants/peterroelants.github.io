...
- IndexNow notifications are manual by design. After a major public page
  addition, update, redirect, or deletion has been deployed and verified, use
  the `indexnow` skill or `just indexnow <production-url> ...` for the affected
  URLs. Do not submit minor tooling, styling, or cosmetic changes. See
  `build_info/indexnow.md` for the protocol notes.
