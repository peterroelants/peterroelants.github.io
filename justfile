...
check: check-convert-scripts check-generated-media check-bokeh-standalone
    git diff --check
    {{bundle}} check
    {{npm}} ls --depth=0
...
# Check extracted notebook media references, alt text, dimensions, and cleanup.
check-generated-media:
    {{uv}} run notebooks/check_generated_media.py

# Check that published Bokeh output is standalone browser JavaScript.
check-bokeh-standalone:
    node --check js/notebook_bokeh_lazy.js
    {{uv}} run notebooks/check_bokeh_standalone.py

# Run focused browser checks against the built static site.
browser-test: build
    {{npm}} run test:browser

# Clear outputs and transient execution metadata from notebooks in place.
clear-notebooks:
    {{uv}} run --with nbformat python notebooks/clear_notebook_metadata.py notebooks
...
# Notify IndexNow about explicitly selected URLs after a major deployed change.
indexnow *urls:
    {{uv}} run tools/indexnow_submit.py {{urls}}
